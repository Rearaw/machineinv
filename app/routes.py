...
def init_app(app):
    ...
    def api_reschedule(equipment_id):
        ...
        latest_service = (
            Service.query.filter_by(equipment_id=equipment_id)
    ...
    def view_category_machines(category_id):
        ...
        return render_template(
            "category_machines.html", category=category, equipments=equipments
        )
    ...
    def add_equipment():
        if request.method == "POST":
            ...
            for image in images:
                if (
                    ...
                    print(
                        f"Saving image: {filename} for equipment: {new_equipment.equipment_id}"
                    )

                    # ✅ Fix: Ensure image reference is added to the database
                    new_image = EquipmentImage(
                        filename=filename, equipment=new_equipment
                    )
        ...
        categories = Category.query.all()
        return render_template(
            "add_equipment.html", locations=locations, categories=categories
        )
    ...
    def service_equipment(equipment_id):
        ...
        if request.method == "POST":
            ...
            service_next_date = (
                datetime.strptime(service_next_date, "%Y-%m-%d").date()
    ...
    def update_equipment(equipment_id):
        ...
        if request.method == "POST":
            ...
            equipment.purchase_date = (
                datetime.strptime(request.form["purchase_date"], "%Y-%m-%d").date()
                if request.form["purchase_date"]
                else None
            )
            equipment.warranty_expiry = (
                datetime.strptime(request.form["warranty_expiry"], "%Y-%m-%d").date()
                if request.form["warranty_expiry"]
                else None
            )
    ...
    def add_location():
        if request.method == "POST":
            ...
            new_location = Location(
                location_name=location_name, location_description=location_description
            )
    ...
    def locations():
        ...
        return render_template(
            "locations.html",
            locations_with_counts_and_machines=locations_with_counts_and_machines,
        )
    ...
    def view_location_machines(location_id):
        ...
        return render_template(
            "location_machines.html", location=category, equipments=equipments
        )
